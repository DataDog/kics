...
def fetch(page=1, max_items=100):
    ...
    url = "https://api.github.com/repos/DataDog/kics/pulls/{}/files?per_page={}page={}".format(
        KICS_PR_NUMBER, max_items, page
    )
...
def validate_queries_metadata(queries):
    ...
    if len(errors) > 0:
        for error in errors:
            print(error)
        exit_with_error(
            "There are metadata files with errors ({} files)".format(len(errors))
        )
