...
CxPolicy[result] {
    ...
    resourceToCheck := common_lib.get_module_equivalent_key("aws", modules.source, resource_type, "instance")
    ...
    result := {
        ...
        "searchKey": sprintf("module[%s].metadata_options.http_tokens", [name]),
        "issueType": "IncorrectValue",
        "keyExpectedValue": "metadata_options.http_tokens should be 'required'",
        "keyActualValue": sprintf("metadata_options.http_tokens is '%s'", [metadata_opt.http_tokens]),
        "searchLine": common_lib.build_search_line(["module", name, "metadata_options", "http_tokens"], []),
        "remediation": json.marshal({
            "before": "optional",
            "after": "required"
        }),
...
CxPolicy[result] {
    ...
    result := {
        ...
        "resourceType": "module",
        "resourceName": sprintf("%s", [name]),
        "searchKey": sprintf("module[%s].metadata_options", [name]),
        "issueType": "MissingValue",
        "keyExpectedValue": "metadata_options must be defined with http_tokens set to 'required'",
        "keyActualValue": "metadata_options is missing, defaulting to IMDSv1",
        "searchLine": common_lib.build_search_line(["module", name, "metadata_options"], []),
        "remediation": "metadata_options {\n\t  http_tokens = \"required\"\n}",
        "remediationType": "addition"
    }
}
